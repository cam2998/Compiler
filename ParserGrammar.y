...
Stmt    : Decl                                     { printf("%s = \n", (char *) $1);};
Stmt    : Assign                                   {};
Decl    : Type IDLst                               { printf("%s = \n", (char *) $2);};
Type    : INT_TOK                                  {$$ = (long) strdup(yytext) ;};
Type    : CHR_TOK                                  {$$ = (long) strdup(yytext) ;};
IDLst   : IDENT_TOK MLst                           {$$ = (long) strdup(yytext) ;};
MLst    : COMMA_TOK IDLst                          { printf("%s := \n", (char *) $2);};
MLst    :                                          {};
Assign  : LHS ASSIGN_TOK Expr                      { printf("%s :=\n",(char *)$1);};
LHS     : IDENT_TOK                                {$$ = (long) strdup(yytext) ;};
Expr		:	Term MExpr                               {};
MExpr		:	AddOp Term MExpr			                   { printf("%s :=\n",(char *)$1);};
MExpr		:															             {};
Term		:	Factor MTerm                             {};
MTerm		:	MultOp Factor MTerm                      { printf("%s :=\n",(char *)$1);};
MTerm		:															             {};
